...
namespace AnniPlayer
{
    public partial class MainWindow : Window
    {
        ...
        private bool _smartFillEnabled = false;
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            _videoPanel.MouseWheel += (s, ev) =>
            {
                Dispatcher.Invoke(() =>
                {
                    ...
                    double step = ev.Delta > 0 ? 5.0 : -5.0;
                    double newVol = Math.Clamp(sliderVolume.Value + step, 0, 200);
                    sliderVolume.Value = newVol;
                    sliderVolumeFS.Value = newVol;
                    string volStr = newVol.ToString(System.Globalization.CultureInfo.InvariantCulture);
                    MpvNative.mpv_command_string(_mpv, $"set volume {volStr}");
                    ShowOsd(string.Format(I18nService.Instance["OsdVolume"], (int)newVol));
                });
            };
        ...
        private void Timer_Tick(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!_isCurrentImage && _hasMedia && !string.IsNullOrEmpty(_currentPlayingFilePath))
                {
                    string eof = MpvGet("eof-reached");
                    if (eof == "yes")
                    {
        ...
        {
            _smartFillEnabled = !_smartFillEnabled;
            if (_smartFillEnabled && _currentAspectRatio != "default")
            {
            ...
            ShowOsd(_smartFillEnabled ? I18nService.Instance["OsdSmartFillOn"] : I18nService.Instance["OsdSmartFillOff"]);
        }
        ...
        private void UpdateSmartFillUI()
        {
            ...
            if (btnSmartFill != null) btnSmartFill.Foreground = _smartFillEnabled ? activeBrush : inactiveBrush;
            if (btnSmartFillFS != null) btnSmartFillFS.Foreground = _smartFillEnabled ? activeBrush : inactiveBrush;
            ...
            if (videoMenu != null)
            {
                ...
                {
                    if (item is System.Windows.Controls.MenuItem mi && mi.Tag?.ToString() == "smartfill")
                    {
                        mi.IsChecked = _smartFillEnabled;
                    }
                }
            }
        }
        ...
        private void UpdateAutoCropUI()
        {
            ...
            UpdateContextMenuChecked(btnAutoCrop?.ContextMenu);
            UpdateContextMenuChecked(btnAutoCropFS?.ContextMenu);
        ...
        private void UpdateContextMenuChecked(System.Windows.Controls.ContextMenu? menu)
        {
            if (menu == null) return;
            foreach (var item in menu.Items)
            {
                if (item is System.Windows.Controls.MenuItem mi)
                {
                    mi.IsChecked = (mi.Tag?.ToString() == _autoCropMode);
                }
            }
        }
        ...
        private void UpdateSmartFill()
        {
            ...
            try
            {
                ...
                if (Math.Abs(vrFill - pr) > 0.05)
                {
                    ...
                    string lavfi = $"lavfi=\"[vid1]{cropFilter}scale={vidW}:{vidH},setsar=1[vid_sq];[vid_sq]split=2[blur][orig];[blur]scale={blurW}:{blurH}:flags=fast_bilinear,boxblur=3:2,scale={targetW}:{targetH}:flags=bilinear[bg];[bg][orig]overlay=x='trunc((W-w)/4)*2':y='trunc((H-h)/4)*2',setsar=1[vo]\"";
        ...
        private void LoadSettingsFromConfig()
        {
            try
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    if (doc.RootElement.TryGetProperty("autoCropMode", out var cropProp))
                    {
                        _autoCropMode = cropProp.GetString() ?? "none";
                    }

                    if (doc.RootElement.TryGetProperty("smartFillEnabled", out var fillProp))
                    {
                        _smartFillEnabled = fillProp.GetBoolean();
                    }
                }
            }
            catch { }
        ...
        {
            sliderVolume.Value = Math.Clamp(sliderVolume.Value + (e.Delta > 0 ? 5 : -5), 0, 200);
        }

        private void Window_PreviewMouseWheel(object sender, MouseWheelEventArgs e)
        {
            if (_mpv == IntPtr.Zero) return;
            // Only handle wheel on the video area (not when hovering over sliders / buttons)
            var hit = Mouse.DirectlyOver;
            bool onVideoArea = hit == null || hit is System.Windows.Forms.Integration.WindowsFormsHost
                || (hit is System.Windows.FrameworkElement fe && fe.Name == "videoGrid");
            if (!onVideoArea) return;

            double step = e.Delta > 0 ? 5.0 : -5.0;
            double newVol = Math.Clamp(sliderVolume.Value + step, 0, 200);
            sliderVolume.Value = newVol;
            sliderVolumeFS.Value = newVol;
            string volStr = newVol.ToString(System.Globalization.CultureInfo.InvariantCulture);
        ...
        private bool ExecutePlayerHotkey(Key realKey, ModifierKeys modifiers)
        {
            ...
            {
                sliderVolume.Value = Math.Min(200, sliderVolume.Value + 5);
                return true;
            }
            ...
            {
                if (sliderBrightness != null) sliderBrightness.Value = Math.Min(100, sliderBrightness.Value + 10);
                return true;
            }
            if (IsHotkeyMatch(currentCombo, hk.BrightnessDown, hk.SecBrightnessDown))
            {
                if (sliderBrightness != null) sliderBrightness.Value = Math.Max(-50, sliderBrightness.Value - 10);
                return true;
            }
